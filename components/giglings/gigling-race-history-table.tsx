...
function placementLabel(participant: RaceParticipant) {
  return typeof participant.finalPosition === "number"
    ? `P${participant.finalPosition}`
    : "Pending";
}

function scoreLabel(participant: RaceParticipant) {
  return typeof participant.performanceScore === "number"
...
function conditionsLabel(race: Race) {
  return [
    formatConditionLabel(race.distance),
    formatConditionLabel(race.weather),
    formatConditionLabel(race.trackCondition)
  ].join(" / ");
}

export function GiglingRaceHistoryTable({
  rows,
  pageSize = 10
}: GiglingRaceHistoryTableProps) {
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-lg border border-white/10 bg-white/[0.03] p-5 text-sm text-white/54">
        No races match this Gigling yet.
      </div>
    );
  }
        ...
        {visibleRows.map(({ participant, race }) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <Link
                  className="text-lg font-black text-cyan-racing transition hover:text-white"
                  href={`/races/${race.id}`}
                >
                  #{race.raceNumber}
                </Link>
                <p className="mt-1 text-xs font-bold uppercase tracking-[0.16em] text-white/38">
                  {formatDateTime(race.startedAt)}
                </p>
              </div>
        ...
        <div className="flex items-center gap-2">
          <button
            className={cn(
              ...
              currentPage === 1
                ? "cursor-not-allowed border-white/8 text-white/28"
                : "border-white/12 text-white/70 hover:border-cyan-racing/35 hover:text-cyan-racing"
            )}
