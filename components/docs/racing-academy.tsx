...
function DocsSection({ children, description, id, kicker, title }: { children: ReactNode; description?: string; id: string; kicker: string; title: string }) {
	return (
		<motion.section id={id} className="scroll-mt-28" initial="hidden" variants={stagger} viewport={{ once: true, margin: "-90px" }} whileInView="show">
...
function AcademyCard({ children, className, icon: Icon, title, tone = "cyan" }: { children: ReactNode; className?: string; icon: LucideIcon; title: string; tone?: Tone }) {
	return (
			...
			<div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_0.82fr] lg:items-center">
				<motion.div animate="show" initial="hidden" variants={stagger}>
					...
					<motion.div className="mt-8 flex flex-col gap-3 sm:flex-row" variants={fadeUp}>
						...
						<Link
							className="inline-flex items-center justify-center gap-2 rounded-lg border border-cyan-racing/35 bg-cyan-racing/10 px-5 py-3 text-sm font-black text-cyan-racing transition hover:bg-cyan-racing/16"
							href="/predictor"
						>
						...
						<Link
							className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/14 bg-white/[0.05] px-5 py-3 text-sm font-black text-white/78 transition hover:border-orange-racing/35 hover:text-orange-racing"
							href="/giglings"
						>
							Explore Giglings
				...
				<motion.div className="speed-card rounded-lg p-4" initial={{ opacity: 0, scale: 0.96, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
					<div className="relative z-10">
						...
						<div className="mt-4 rounded-lg border border-violet-racing/24 bg-violet-racing/10 p-4">
							...
							<p className="mt-2 text-sm leading-6 text-white/58">Convert race data into practical choices: enter, wait, swap Gigling, scout rivals, or share the result.</p>
						</div>
					</div>
				</motion.div>
			</div>
...
function DocsTopNav() {
	return (
		<header className="sticky top-0 z-40 border-b border-white/10 bg-[#05070d]/82 px-4 py-3 shadow-[0_18px_60px_rgba(0,0,0,0.32)] backdrop-blur-2xl sm:px-6 lg:px-8 rounded-xl">
			<div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
				...
				<nav className="flex items-center gap-2" aria-label="Docs navigation">
					...
					<Link className="glow-button inline-flex items-center gap-2 rounded-lg bg-violet-racing px-4 py-2 text-sm font-black text-white transition hover:scale-[1.02]" href="/dashboard">
						Open App
						<ArrowRight className="h-4 w-4" />
					</Link>
				</nav>
		...
		<div className="grid gap-3 lg:grid-cols-6">
			{steps.map((step, index) => (
				<motion.div key={step} className="relative rounded-lg border border-white/10 bg-white/[0.04] p-4" variants={fadeUp}>
					<span className="flex h-8 w-8 items-center justify-center rounded-lg border border-orange-racing/28 bg-orange-racing/10 text-sm font-black text-orange-racing">{index + 1}</span>
	...
	return (
		<motion.div className="premium-panel rounded-lg p-5" variants={fadeUp}>
			<div className="relative z-10">
				...
				<p className="mt-4 text-sm leading-6 text-white/56">The output is normalized into win probability, podium probability, confidence, risk, warnings, and plain-English recommendations.</p>
			</div>
		</motion.div>
	);
}
...
function ArchitectureFlow() {
	const nodes = ["Gigaverse APIs / Contracts", "Data Clients", "Adapters", "Typed App Data", "Analytics + Prediction Engines", "Player-Facing UI"];

	return (
		<div className="space-y-3">
			{nodes.map((node, index) => (
				<motion.div key={node} className="flex items-center gap-3" variants={fadeUp}>
					<div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-cyan-racing/28 bg-cyan-racing/10 text-sm font-black text-cyan-racing">{index + 1}</div>
		...
		<section className="relative overflow-hidden rounded-lg border border-violet-racing/30 bg-[linear-gradient(135deg,rgba(168,85,247,0.24),rgba(32,247,255,0.12)_48%,rgba(255,138,31,0.16))] p-7 shadow-glow sm:p-10">
			...
			<div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
				<div>
					...
					<p className="mt-3 max-w-2xl text-sm leading-6 text-white/64">Use the Academy as your manual, then move into the live tools to scout Giglings, inspect races, and run decision support.</p>
				</div>
			...
			<div className="mx-auto mt-5 max-w-7xl space-y-8">
				...
				<div className="grid gap-8 xl:grid-cols-[15rem_1fr]">
					...
					<div className="space-y-16">
						...
						<DocsSection description="Post-race explanation turns a loss into a useful adjustment instead of a dead end." id="why-lose" kicker="Why Did I Lose Guide" title="Convert Losses Into Lessons">
							<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
						...
						>
							<div className="grid gap-4 lg:grid-cols-3">
								{metaSignals.map((signal) => (
									<AcademyCard
										key={signal.label}
										icon={signal.label === "Emerging Meta" ? Zap : signal.label === "Faction Surge" ? Trophy : LineChart}
										title={signal.label}
										tone={signal.tone}
									>
										{signal.text}
						...
						<DocsSection description="Racing is more memorable when relationships emerge from repeated matchups." id="rivalries" kicker="Rivalry Intelligence Guide" title="Turn Matchups Into Stories">
						...
						<DocsSection description="Reports package race intelligence into social-ready cards and copy blocks." id="reports-sharing" kicker="Reports & Sharing" title="Share The Race Story">
							<div className="grid gap-4 md:grid-cols-3">
