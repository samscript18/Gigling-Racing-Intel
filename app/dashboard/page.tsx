...
import { getDashboardRaceMix, getFactionPerformanceFromRaces, getFactionDashboardData, getHighestWinRateGigling, getRaceConditionTrend, getTopFaction } from "@/lib/gigaverse/analytics";
...
export default async function DashboardPage() {
	...
	if (races.length === 0 || giglings.length === 0) {
		return (
			<div>
				<PageHeader description="Live command center for race conditions, top Giglings, active lobbies, and meta pressure." eyebrow="Race Command" title="Dashboard" />
				<EmptyState
	...
	const activeRaces = races.filter((race) => race.status === "live" || race.status === "scheduled");
	const recentCompletedRaces = races.filter((race) => race.status === "completed").sort((first, second) => new Date(second.endedAt ?? second.startedAt ?? 0).getTime() - new Date(first.endedAt ?? first.startedAt ?? 0).getTime());
	const leaderboardGiglings = [...giglings].sort((first, second) => second.winRate - first.winRate);
	...
	const averageConditionScore = raceTrendData.reduce((total, entry) => total + entry.conditionScore, 0) / Math.max(raceTrendData.length, 1);
	...
	return (
		<div>
				...
				featuredRace={
					featuredRace
						? {
								...
								id: featuredRace.id,
								itemActions: featuredRace.participants.reduce((total, participant) => total + participant.itemsUsed.length, 0),
			...
			<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
				<MetricCard detail={`${raceMix.statusCounts.live} live / ${raceMix.statusCounts.scheduled} scheduled`} icon="flag" label="Races Tracked" mechanic="raceStatus" value={`${races.length}`} />
				<MetricCard detail={`${topFaction.faction} faction leads indexed races`} icon="barChart" label="Top Faction" mechanic="faction" tone="emerald" value={formatPercent(topFaction.winRate)} />
				<MetricCard detail={topGigling.name} icon="trophy" label="Highest Win Rate" mechanic="winRate" tone="orange" value={formatPercent(topGigling.winRate)} />
				<MetricCard detail={`${raceMix.itemPressure} item actions detected`} icon="radar" label="Prize Flow" mechanic="prizePool" tone="violet" value={formatToken(raceMix.totalPrizePool)} />
			</div>

			<div className="mt-6 space-y-6">
				<section>
					<SectionHeader description="Live and scheduled lobbies that deserve pre-race attention." title="Active Race Radar" />
				...
				<section>
					<SectionHeader description="Fast paths for the most common Gigling Racing decisions." title="Quick Actions" />
			...
			<div className="mt-6 grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
				...
				<section className="premium-panel rounded-lg p-5">
					<div className="relative z-10">
						<SectionHeader description="A compressed read of race risk, prize pressure, and meta signals for judges opening the dashboard." title="Weekly Race Brief" />
						<div className="grid gap-3 md:grid-cols-3">
							<div className="rounded-lg border border-cyan-racing/22 bg-cyan-racing/8 p-4">
								<p className="text-xs uppercase tracking-[0.18em] text-cyan-racing">Completed</p>
							...
							<div className="rounded-lg border border-orange-racing/22 bg-orange-racing/8 p-4">
								<p className="text-xs uppercase tracking-[0.18em] text-orange-racing">Volatility</p>
								<p className="mt-2 text-3xl font-black text-white">{Math.round(averageConditionScore)}</p>
							...
							<div className="rounded-lg border border-emerald-racing/22 bg-emerald-racing/8 p-4">
								<p className="text-xs uppercase tracking-[0.18em] text-emerald-racing">Best Read</p>
								<p className="mt-2 text-3xl font-black text-white">{formatPercent(topFaction.podiumRate)}</p>
								<p className="mt-1 text-sm capitalize text-white/48">{topFaction.faction} podium rate</p>
							</div>
						</div>
						<p className="mt-5 rounded-lg border border-white/10 bg-white/[0.035] p-4 text-sm leading-6 text-white/58">
							...
							{` ${topGigling.name}`} owns the highest individual win rate, and hot or cold races should be treated as higher-risk predictor inputs.
						</p>
					</div>
				</section>
			</div>
